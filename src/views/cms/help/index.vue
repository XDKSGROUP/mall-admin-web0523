  ...
  <div class="app-container">
      ...
      <el-table ref="infoTable" :data="list" style="width: 100%;" v-loading="listLoading" border>
        ...
        <el-table-column label="类别" width="120" align="center">
          <template slot-scope="scope">
            <div style="text-align: left;">{{ scope.row.categoryName }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="浏览数量" width="120" align="center">
    ...
    <el-dialog title="详细信息" :visible.sync="dialogDetailVisible">
      ...
      <div class="table-layout">
        ...
        <el-row>
          ...
          <el-col :span="4" class="table-cell">{{ info.readCount }}</el-col>
          <el-col :span="4" class="table-cell">{{ getShowStatus(info.showStatus) }}</el-col>
          <el-col :span="4" class="table-cell">{{ info.createTime }}</el-col>
        </el-row>
        <el-row>
          <el-col :span="24" class="table-cell-title">内容</el-col>
        </el-row>
        <el-row>
          <el-col :span="24" class="table-cell">{{ info.content }}</el-col>
        </el-row>
      </div>
    ...
    <el-dialog :title="isEdit ? '编辑' : '添加'" :visible.sync="dialogVisible">
      <el-form :model="info" ref="infoForm" :rules="infoRules" label-width="120px" size="small">
        ...
        <el-form-item prop="content" label="内容：" class="row">
          <tinymce :height="300" v-model="info.content" placeholder="请输入"></tinymce>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
<script>
import { listInfo, addInfo, setInfo } from '@/api/help';
import { listInfo as classListInfo } from '@/api/helpCategory';
...
export default {
  ...
  methods: {
    async getClassList() {
      ...
      me.category.list.push(...get(rst.data.list));
    },
    getShowStatus(value) {
      value = value + "";
