  ...
  <div class="app-container">
      ...
      <div class="search">
        <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
          <el-form-item label="编号：">
            <el-input size="mini" v-model="listQuery.id" class="input-width" placeholder="请输入" clearable></el-input>
          </el-form-item>
        </el-form>
      </div>
    </el-card>
    ...
    <div class="table-container">
      <el-table ref="infoTable" :data="list" style="width: 100%;" v-loading="listLoading" border>
        ...
        <el-table-column label="等级名称" width="90" align="center">
          <template slot-scope="scope">{{ scope.row.name }}</template>
        </el-table-column>
        <el-table-column label="默认值" width="80" align="center">
          <template slot-scope="scope">{{ getYesNo(scope.row.defaultStatus) }}</template>
        </el-table-column>
        <el-table-column label="升级条件" width="190" align="center">
          <template slot-scope="scope">{{ scope.row.upgradeConditions }}</template>
        </el-table-column>
        <el-table-column label="权益1" width="180" align="center">
          <template slot-scope="scope">{{ scope.row.rightsFirst }}</template>
        </el-table-column>
        <el-table-column label="权益2" width="160" align="center">
          <template slot-scope="scope">{{ scope.row.rightsSecond }}</template>
        </el-table-column>
        <el-table-column label="是否购买特定商品" width="135" align="center">
          <template slot-scope="scope">{{ getYesNo(scope.row.isBuySpecific) }}</template>
        </el-table-column>
        <el-table-column label="是否需要实名" width="105" align="center">
          <template slot-scope="scope">{{ getYesNo(scope.row.isRealName) }}</template>
        </el-table-column>
        <el-table-column label="直推有效会员数" width="120" align="center">
          <template slot-scope="scope">{{ scope.row.recommendEffectiveMemberCount }}</template>
        </el-table-column>
...
<script>
import { listInfo} from '@/api/memberLevel';
import { formatDate } from '@/utils/date';
import { enumMoneyType, enumYesNo } from "@/utils/enums";
