    ...
    <div class="table-container">
      <el-table ref="infoTable" :data="list" style="width: 100%;" v-loading="listLoading" border>
        ...
        <el-table-column label="捐赠数量" width="120" align="center">
          <template slot-scope="scope">{{ scope.row.donationAmount }}</template>
        </el-table-column>
        <el-table-column label="项目名称" width="120" align="center">
          <template slot-scope="scope">{{ scope.row.projectName }}</template>
        </el-table-column>
        <el-table-column label="项目图片" width="100" align="center">
          <template slot-scope="scope"><el-image style="width: 80px; height: 80px" :src="scope.row.projectPic"
              fit="contain"></el-image></template>
        </el-table-column>
        <el-table-column label="善款接收方" width="120" align="center">
          <template slot-scope="scope">{{ scope.row.projectDonationsReceiver }}</template>
        </el-table-column>
        <el-table-column label="执行机构" width="120" align="center">
          <template slot-scope="scope">{{ scope.row.projectActuator }}</template>
        </el-table-column>
        <el-table-column label="目标金额" width="120" align="center">
          <template slot-scope="scope">{{ scope.row.projectTargetMoney }}</template>
        </el-table-column>
        <el-table-column label="已筹金额" width="120" align="center">
          <template slot-scope="scope">{{ scope.row.projectRaisedMoney }}</template>
        </el-table-column>
        <el-table-column label="捐赠份数" width="120" align="center">
          <template slot-scope="scope">{{ scope.row.projectDonationsNumber }}</template>
        </el-table-column>
        <el-table-column label="关注人数" width="120" align="center">
          <template slot-scope="scope">{{ scope.row.projectFollowersNumber }}</template>
        </el-table-column>
        <el-table-column label="捐赠时间" width="160" align="center">
          <template slot-scope="scope">{{ scope.row.donationTime | formatDateTime }}</template>
        </el-table-column>
...
<script>
import { listInfo, addInfo, setInfo, setStatus, delInfo, paySuccess, authAbolish,exportExcel } from '@/api/contribute';
...
export default {
  ...
  methods: {
    ...
    handleSearchList() {
      ...
      q.pageNum = 1;
      if (q.donationTime.length) {
        q.donationTimeStart = q.donationTime[0];
        q.donationTimeEnd = q.donationTime[1];
      }
      me.getList();
    },
