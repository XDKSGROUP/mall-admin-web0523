<template>
  <div style="margin-top: 50px">
    <el-form :model="value" ref="productAttrForm" label-width="120px" class="form-inner-container" size="small">
      <el-form-item prop="productAttributeCategoryId" :rules="[{ required: true, message: '请选择属性类型', trigger: 'change' }]"
        label="属性类型：">
      ...
      <el-form-item prop="pic" :rules="[{ required: true, message: '请上传商品相册', trigger: 'change' }]" label="商品相册：">
...
>>>>>>> 3414240f99cd0f824d9609581f5907f66d4ae6df
      }
    ...
    handleNext() {
      ...
      me.$refs.productAttrForm.validate(function (ok) {
        ...
        for (let i = 0, c = me.value.skuStockList.length; i < c; i++) {
          ...
          let name = [];
          arr.forEach(it => {
            name.push(it.key + "-" + it.value);
          });
          name = name.join(", ") + "的";
          if (!it.price) {
            me.msg("请填写" + name + "销售价格");
            return;
          }
          if (!it.promotionPrice) {
            me.msg("请填写" + name + "促销价格");
            return;
          }
          if (!it.stock) {
            me.msg("请填写" + name + "商品库存");
            return;
          }
          if (!it.lowStock) {
            me.msg("请填写" + name + "库存预警值");
            return;
          }
          if (!it.promotionPrice) {
    ...
    msg(msg) {
      this.$message({ message: msg, type: 'warning', duration: 1000 });
    }
  },
  created() {
    this.getProductAttrCateList();
